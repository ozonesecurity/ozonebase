...
void local_jpeg_error_exit( j_common_ptr cinfo )
{
	...
	local_error_ptr local_err = (local_error_ptr)cinfo->err;

	(local_err->pub.format_message)( cinfo, buffer ); 
	...
	longjmp( local_err->setjmp_buffer, 1 );
}
...
void local_jpeg_emit_message( j_common_ptr cinfo, int msg_level )
{
	...
	if ( msg_level < 0 )
	{
		 ...
		 */
		if ( local_err->pub.num_warnings == 0 || local_err->pub.trace_level >= 3 )
		{
			(local_err->pub.format_message)( cinfo, buffer ); 
			Warning( "%s", buffer );
		}
		/* Always count warnings in num_warnings. */
		local_err->pub.num_warnings++;
	}
	else
	{
		/* It's a trace message.  Show it if trace_level >= msg_level. */
		if ( local_err->pub.trace_level >= msg_level )
		{
			(local_err->pub.format_message)( cinfo, buffer ); 
